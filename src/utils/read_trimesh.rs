...
pub fn load_trimesh_from_ply(ply_file_path: &str) -> Result<TriMesh, Box<dyn Error>> {
    load_trimesh(ply_file_path, 1.0).map_err(|e| Box::new(io::Error::other(e)) as Box<dyn Error>)
}

/// Function to load a TriMesh from an STL file
#[deprecated(since = "1.8.4", note = "Use rs_read_trimesh::load_trimesh instead")]
pub fn load_trimesh_from_stl(stl_file_path: &str) -> Result<TriMesh, Box<dyn Error>> {
    load_trimesh(stl_file_path, 1.0).map_err(|e| Box::new(io::Error::other(e)) as Box<dyn Error>)
}
